...
- `DISTORTION_CORRECTION_MODE="phasediff"` supports BIDS `*_phasediff.nii.gz` plus `*_magnitude1/2.nii.gz` field maps through the HCP `FieldMapPreprocessingAll.sh`/`fsl_prepare_fieldmap` path.
...
The wrapper config currently defaults to the BIDS phasediff field-map path:

```bash
DISTORTION_CORRECTION_MODE="phasediff"
```

Other modes:

- `DISTORTION_CORRECTION_MODE="topup"` uses AP/PA spin-echo EPI field maps and TOPUP.
- `DISTORTION_CORRECTION_MODE="phasediff"` imports BIDS `*_phasediff.nii.gz`, `*_magnitude1.nii.gz`, and optional `*_magnitude2.nii.gz` as `PhaseDiff_S*_R*` and `Magnitude*_S*_R*`, then runs HCP `FieldMapPreprocessingAll.sh --method=SiemensFieldMap` to produce rad/s field maps.
...
Single-echo BOLD files without an `echo-<N>` entity are imported as `E1`.
On Windows, `--mode symlink` requires symlink privileges; if Windows blocks symlinks and hardlinks, rerun with `--mode copy`.
