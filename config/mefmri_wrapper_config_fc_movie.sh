...
START_SESSION=1
START_FROM_MODULE="fc_movie"   # validate|anat_hcp|anat_charm|fieldmaps|coreg|headmotion|denoise|mgtr|vol2surf|concat|fc_movie|nsi|pfm|pfm_update
STOP_AFTER_MODULE="fc_movie"           # "" to run full chain
...
# -----------------------------------------------------------------------------
HCP_ANAT_CLEAN_START=0           # 1 removes prior anat outputs before rerun
HCP_REGNAME="MSMSulc"            # MSMSulc|FS ; set FS if MSM isn't available
CHARM_REUSE_EXISTING_M2M=1       # 1 reuses existing anat/m2m_<Subject> outputs if present
